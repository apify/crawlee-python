...
from crawlee.crawlers._pydantic_ai._prompts import _CLEAN_HTML_PROMPT_NOTES, _TRUNCATION_MARKER
...
def test_prompt_notes() -> None:
    assert PydanticAiCleanHtmlDistiller().get_prompt_notes() == _CLEAN_HTML_PROMPT_NOTES
    assert PydanticAiCleanHtmlDistiller(prompt_notes=None).get_prompt_notes() is None
    assert PydanticAiCleanHtmlDistiller(prompt_notes='custom').get_prompt_notes() == 'custom'


def test_keeps_text_and_semantic_attributes() -> None:
    html = '<div class="card" itemprop="product" data-id="7"><a href="/p">Item</a></div>'
    distilled_html = PydanticAiCleanHtmlDistiller().distill(html)

    assert html == distilled_html


def test_drops_non_semantic_attributes() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller().distill(
...
def test_drops_noise_tags() -> None:
    distiller = PydanticAiCleanHtmlDistiller()
    distilled_html = distiller.distill('<div><svg><path/></svg><noscript>x</noscript><p>Item</p></div>')
...
def test_drops_data_uri_attribute() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller().distill('<img src="data:image/png;base64,AAAABBBB" alt="logo">')

    assert distilled_html == '<img alt="logo">'


def test_limited_class_attribute() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller(max_classes=2).distill('<div class="a b c d e">x</div>')

    assert distilled_html == '<div class="a b">x</div>'


def test_drops_empty_class_attribute() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller().distill('<div class="   ">x</div>')

    assert distilled_html == '<div>x</div>'


def test_truncates_long_attribute_values() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller(max_attr_len=5).distill(f'<a href="{"x" * 50}">link</a>')

    assert distilled_html == f'<a href="xxxxx{_TRUNCATION_MARKER}">link</a>'


def test_truncates_json_payload() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller(max_json_len=5).distill(
...
def test_drops_head() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller(keep_head=False).distill(
...
def test_normalizes_whitespace() -> None:
    distilled_html = PydanticAiCleanHtmlDistiller().distill('<p>a    b\n\n\tc</p>')
...
def test_enforces_max_size(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger='crawlee.crawlers._pydantic_ai._clean_html_distiller'):
        out = PydanticAiCleanHtmlDistiller(max_size=50).distill(html)
