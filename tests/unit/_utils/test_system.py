...
def cpu_load(monkeypatch: pytest.MonkeyPatch) -> Mock:
    """Replace the CPU readings taken against a limit, so that no real limit is measured."""
    sampler = Mock(spec=proclimits.CpuLoad)
    ...
    monkeypatch.setattr(proclimits, 'get_cpu_used_ratio', Mock(return_value=None))
    monkeypatch.setattr(proclimits, 'get_cpu_limit', Mock(return_value=None))
    return sampler
...
def fake_snapshot(
    *, memory_budget: proclimits.MemoryBudget | None = None, cpu_limit: float | None = None
) -> proclimits.Snapshot:
    """Stand in for `proclimits.snapshot()`, describing an unrestricted process unless told otherwise."""
    return proclimits.Snapshot(memory_budget=memory_budget, cpu_limit=cpu_limit, cpu_usage=None)
...
def test_get_cpu_info_returns_valid_values(cpu_load: Mock) -> None:
    cpu_info = get_cpu_info(cpu_load)
...
def test_get_cpu_info_measures_against_the_limit(monkeypatch: pytest.MonkeyPatch, cpu_load: Mock) -> None:
    ...
    assert get_cpu_info(cpu_load).used_ratio == 0.5
...
def test_get_cpu_info_measures_a_window_when_the_sampler_has_no_reading(
    ...
    assert get_cpu_info(cpu_load).used_ratio == 0.25
...
def test_get_cpu_info_reports_an_idle_limit_as_no_load(
    ...
    monkeypatch.setattr(proclimits, 'get_cpu_used_ratio', Mock(return_value=measured))
    cpu_percent = Mock(return_value=42.0)
    monkeypatch.setattr(psutil, 'cpu_percent', cpu_percent)

    assert get_cpu_info(cpu_load).used_ratio == 0.0
    cpu_percent.assert_not_called()
...
def test_get_cpu_info_falls_back_to_the_host_without_a_rate(monkeypatch: pytest.MonkeyPatch, cpu_load: Mock) -> None:
    ...
    assert get_cpu_info(cpu_load).used_ratio == 0.42
...
def test_log_resource_limits_reports_what_applies(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
    memory_budget: proclimits.MemoryBudget | None,
    ...
    monkeypatch.setattr(proclimits, 'snapshot', Mock(return_value=snapshot))
    ...
    if expected_message is None:
        ...
        assert any(expected_message in message for message in reported)


@pytest.mark.usefixtures('measured_current_process')
def test_log_resource_limits_reports_once(monkeypatch: pytest.MonkeyPatch) -> None:
    """The limits are reported on the first sample only, not on every one."""
    snapshot = Mock(return_value=fake_snapshot())
    monkeypatch.setattr(proclimits, 'snapshot', snapshot)
