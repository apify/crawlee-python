...
async def test_proxy(
    ...
    url = str(httpbin.with_path('/status/222', keep_query=True, keep_fragment=True))
...
async def test_common_headers_and_user_agent(httpbin: URL) -> None:
    ...
    response = await client.send_request(str(httpbin.with_path('/get', keep_query=True, keep_fragment=True)))
