...
async def test_basic_request(httpbin: URL) -> None:
    requests = [str(httpbin.with_path('/', keep_query=True, keep_fragment=True))]
...
async def test_chromium_headless_headers(httpbin: URL) -> None:
    ...
    await crawler.run([str(httpbin.with_path('/get', keep_query=True, keep_fragment=True))])
...
async def test_custom_headers(httpbin: URL) -> None:
    ...
    await crawler.run(
        [Request.from_url(str(httpbin.with_path('/get', keep_query=True, keep_fragment=True)), headers=request_headers)]
    )
