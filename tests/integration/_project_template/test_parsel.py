...
@pytest.fixture
def apify_path() -> Path:
    # TODO: Solve on platform
    return Path('/home/pijukatel/.nvm/versions/node/v22.11.0/bin/apify')

@pytest.fixture
def node_path() -> Path:
    # TODO: Solve on platform
    return Path('/home/pijukatel/.nvm/versions/node/v22.11.0/bin/node')

# TODO: resolve on platform
test_user = 'apify-test-python-sdk' # Maybe get new user for this test suite?
test_user = 'josef.prochazka'
apify_token = os.getenv('APIFY_TEST_USER_API_TOKEN')
...
async def test_default_template_actor_at_apify(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, apify_path: Path, node_path:Path) -> None:
    with _change_dir(tmp_path):
        ...
        with _change_dir(tmp_path/actor_name):
            # Actor init
            init_process = subprocess.Popen([str(node_path), str(apify_path), 'init'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            await asyncio.sleep(1)
            init_stdout, init_stderr = init_process.communicate(f'{actor_name}\n'.encode())

            # Actor push
            build_process = subprocess.run([str(node_path), str(apify_path), 'push'], capture_output=True, check=False)
    ...
    assert ('Crawler.stop() was called with following reason: '
            'The crawler has reached its limit of 50 requests per crawl.') in actor_run_log
