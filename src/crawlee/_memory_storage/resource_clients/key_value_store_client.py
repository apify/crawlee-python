...
class KeyValueStoreClient(BaseResourceClient):
    ...
    def __init__(
        ...
        *,
        ...
        self.base_storage_directory = base_storage_directory
        self.memory_storage_client = memory_storage_client
        self.id = id_ or crypto_random_object_id()
        self.name = name
        self.created_at = created_at or datetime.now(timezone.utc)
        self.accessed_at = accessed_at or datetime.now(timezone.utc)
        self.modified_at = modified_at or datetime.now(timezone.utc)

        self.resource_directory = os.path.join(self.base_storage_directory, self.name or self.id)
        self.records: dict[str, KeyValueStoreRecord] = {}
        self.file_operation_lock = asyncio.Lock()

    def _get_storage_dir(self) -> str:
        return self.memory_storage_client.key_value_stores_directory

    def _get_storage_client_cache(self) -> list[KeyValueStoreClient]:
        return self.memory_storage_client.key_value_stores_handled
    ...
    def _create_from_directory(
        ...
        for entry in os.scandir(storage_directory):
            ...
            if os.path.exists(os.path.join(storage_directory, entry.name + '.__metadata__.json')):
                with open(
                    ...
                    try:
                        ...
                        logger.warning(
                            f'Failed to parse metadata for "{entry.name}" in store {name or id_}.',
            ...
            try:
                ...
                logger.warning(
                    f"""Key-value store entry "{metadata['key']}" for store {name or id_} could not be parsed."""
    ...
    async def get(self) -> dict | None:
        ...
        """
        client = self.find_or_create_client_by_id_or_name(
            memory_storage_client=self.memory_storage_client,
            id_=self.id,
            name=self.name,
        )

        if client:
            async with client.file_operation_lock:
                await client.update_timestamps(has_been_modified=False)
                return client.to_resource_info()
    ...
    async def update(self, *, name: str | None = None) -> dict:
        ...
        existing_store_by_id = self.find_or_create_client_by_id_or_name(
            memory_storage_client=self.memory_storage_client, id_=self.id, name=self.name
        )

        if existing_store_by_id is None:
            raise_on_non_existing_storage(Storage.KEY_VALUE_STORE, self.id)
        ...
        async with existing_store_by_id.file_operation_lock:
            ...
            existing_store_by_name = next(
                (
                    store
                    for store in self.memory_storage_client.key_value_stores_handled
            ...
            if existing_store_by_name is not None:
                raise_on_duplicate_storage(Storage.KEY_VALUE_STORE, 'name', name)
            ...
            existing_store_by_id.resource_directory = os.path.join(
                self.memory_storage_client.key_value_stores_directory,
                name,
            )
    ...
    async def delete(self) -> None:
        ...
        store = next(
            (store for store in self.memory_storage_client.key_value_stores_handled if store.id == self.id), None
        )

        if store is not None:
            async with store.file_operation_lock:
                self.memory_storage_client.key_value_stores_handled.remove(store)
                store.records.clear()
    ...
    async def set_record(self, key: str, value: Any, content_type: str | None = None) -> None:
        ...
        async with existing_store_by_id.file_operation_lock:
            ...
            if self.memory_storage_client.persist_storage:
    ...
    async def persist_record(self, record: KeyValueStoreRecord) -> None:
        ...
        if self.memory_storage_client.write_metadata:
    ...
    async def update_timestamps(self, has_been_modified: bool) -> None:  # noqa: FBT001
        ...
        await update_metadata(
            ...
            write_metadata=self.memory_storage_client.write_metadata,
        )
