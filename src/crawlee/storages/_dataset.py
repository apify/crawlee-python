...
class ExportDataJsonKwargs(TypedDict):
    ...
    Args:
        skipkeys: If True (default: False), then dict keys that are not of a basic type (str, int, float, bool, None)
            will be skipped instead of raising a TypeError.
        ensure_ascii: Whether non-ASCII characters should be escaped in the output JSON string.
        check_circular: If False (default: True), then the circular reference check for container types will be skipped
            and a circular reference will result in a RecursionError (or worse).
        allow_nan: If False (default: True), then it will be a ValueError to serialize out of range float values
            (nan, inf, -inf) in strict compliance of the JSON specification. If allow_nan is true, their JavaScript
            equivalents (NaN, Infinity, -Infinity) will be used.
        cls: Allows using a custom JSON encoder
        indent: Specifies the number of spaces to use for indentation in the pretty-printed JSON output.
            separators: a (item_separator, key_separator) tuple. The default is (', ', ': ') if indent is None
            and (',', ': ') otherwise.
...
class ExportDataCsvKwargs(TypedDict):
    r"""Keyword arguments for dataset's `export_data_csv` method.
    ...
    Args:
        ...
        doublequote: Controls how instances of quotechar appearing inside a field should themselves be quoted.
            ...
            It defaults to True.
        escapechar: A one-character string used by the writer to escape the delimiter if quoting is set to QUOTE_NONE
            and the quotechar if doublequote is False. On reading, the escapechar removes any special meaning from
