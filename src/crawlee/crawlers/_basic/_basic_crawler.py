...
class BasicCrawler(Generic[TCrawlingContext, TStatisticsState]):
    ...
    async def get_request_queue(
        ...
        *,
        ...
        """Return the `RequestQueue` with the given ID or name. If none is provided, return the default one."""
        return await RequestQueue.open(id=id, name=name, alias=alias)
    ...
    async def add_requests(
        ...
        *,
        ...
        Args:
            ...
            rq_name: Name of the request queue to add requests to (global scope).
            rq_alias: Alias of the request queue to add requests to (run scope, unnamed).
        """
        allowed_requests = []
        skipped = []
        ...
        # Determine which request queue to use
        if rq_name or rq_alias:
            request_manager = await self.get_request_queue(name=rq_name, alias=rq_alias)
        else:
            request_manager = await self.get_request_manager()
    ...
    async def _commit_request_handler_result(self, context: BasicCrawlingContext) -> None:
        ...
        for add_requests_call in result.add_requests_calls:
            ...
            # Determine target request queue based on rq_name or rq_alias
            rq_name = enqueue_links_kwargs.get('rq_name')
            rq_alias = enqueue_links_kwargs.get('rq_alias')

            if rq_name or rq_alias:
                # Use targeted request queue
                target_request_queue = await self.get_request_queue(name=rq_name, alias=rq_alias)
                await target_request_queue.add_requests(requests)
            else:
                # Use default request queue
