...
# TODO:
# I am not sure about the file extension inference.
...
class FileSystemKeyValueStoreClient(KeyValueStoreClient):
    ...
    async def get_value(self, *, key: str) -> KeyValueStoreRecord | None:
        # Try to infer potential filenames for this key with various extensions
        possible_filenames = [key]
        possible_filenames.extend(f'{key}{ext}' for ext in mimetypes.types_map.values() if not key.endswith(ext))
        ...
        if len(possible_filenames) > 1:
            logger.warning(
                f'KVS structure is malformed, key "{key}" has multiple possible filenames: {possible_filenames}.'
            )
            return None
    ...
    async def set_value(self, *, key: str, value: Any, content_type: str | None = None) -> None:
        ...
        async with self._lock:
            ...
            # Update the store metadata to record the access and modification.
            await self._update_metadata(update_accessed_at=True, update_modified_at=True)

    @override
    async def delete_value(self, *, key: str) -> None:
        # Try to find the file with all possible extensions
        possible_filenames = [key]
        possible_filenames.extend(f'{key}{ext}' for ext in mimetypes.types_map.values() if not key.endswith(ext))
        ...
        async with self._lock:
            ...
            # If we deleted something, update the store metadata
            if deleted:
                await self._update_metadata(update_accessed_at=True, update_modified_at=True)
