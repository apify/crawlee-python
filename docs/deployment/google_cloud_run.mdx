...
For deploying crawlers based on headless browsers or applications requiring more granular control than [GCP Cloud Functions](./gcp-functions) can provide, Cloud Run is the recommended solution for Python-based Crawlee applications.

GCP Cloud Run allows you to deploy using Docker containers, giving you full control over your environment and the flexibility to use any web server framework of your choice, unlike [Cloud Functions](./gcp-functions) which are limited to [Flask](https://flask.palletsprojects.com/en/stable/). Cloud Run containers are spun up on-demand, so you're only billed for the time it takes to process each request, while also providing a better developer experience by allowing you to test your containers locally before deploying.
