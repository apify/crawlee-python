...
Crawlee builds on the standard Python [`logging`](https://docs.python.org/3/library/logging.html) module. Each crawler instance has its own logger, named after the crawler class (for example `HttpCrawler`), and inside a request handler the same logger is available as `context.log`, so messages from your handlers carry the request context.

## Default configuration

By default, a crawler sets up the logging infrastructure automatically when it is created - it attaches a handler with a human-readable, colored formatter to the root logger, unless the root logger already has handlers configured. You can opt out of this by passing `configure_logging=False` to the crawler's constructor (see <ApiLink to="class/BasicCrawlerOptions">`BasicCrawlerOptions`</ApiLink>) and configure logging yourself.
