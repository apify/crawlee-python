...
import SanityCheckExample from '!!raw-loader!./code/04_sanity_check.py';
...
- Can I scrape it only with HTTP requests (read "with some <ApiLink to="class/_HttpCrawler">`_HttpCrawler`</ApiLink>, e.g. <ApiLink to="class/BeautifulSoupCrawler">`BeautifulSoupCrawler`</ApiLink>")?
...
For the purposes of this tutorial, let's assume that the website cannot be scraped with <ApiLink to="class/_HttpCrawler">`_HttpCrawler`</ApiLink>. It actually can, but we would have to dive a bit deeper than this introductory guide allows. So for now we will make things easier for you, scrape it with <ApiLink to="class/PlaywrightCrawler">`PlaywrightCrawler`</ApiLink>, and you'll learn about headless browsers in the process.
