...
Camoufox is an external tool and is not part of Crawlee - [install it separately](https://pypi.org/project/camoufox/). Note that Camoufox uses a custom build of Firefox which can be hundreds of MB large. You can pre-download it with `python3 -m camoufox fetch`; otherwise Camoufox downloads it automatically on first run. See the [Camoufox Python interface documentation](https://github.com/daijro/camoufox/tree/main/pythonlib#camoufox-python-interface) for details.
