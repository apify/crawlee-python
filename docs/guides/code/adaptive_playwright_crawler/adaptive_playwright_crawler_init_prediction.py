...
from crawlee.crawlers import AdaptivePlaywrightCrawler, RenderingType, RenderingTypePrediction, RenderingTypePredictor
...
class CustomRenderingTypePredictor(RenderingTypePredictor):
    ...
    def predict(self, request: Request) -> RenderingTypePrediction:
        ...
        return RenderingTypePrediction(
            ...
            # Recommends that both sub crawlers should run with 20% chance.
            # When both sub crawlers are running, the predictor can compare results and learn.
            # When this number is high, then predictor is not very confident about the `rendering_type`.
            # When this number is low, then predictor is very confident about the `rendering_type`.
...
def result_checker(result: RequestHandlerRunResult) -> bool:
    # Some function that inspects produced `result` and returns `True` if the result is correct.
