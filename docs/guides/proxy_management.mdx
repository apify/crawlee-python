...
`HttpCrawler`, `BeautifulSoupCrawler` and `PlaywrightCrawler` grant access to information about the currently used proxy in their `request_handler` using a <ApiLink to="class/ProxyInfo">`proxy_info`</ApiLink> object. With the `proxy_info` object, we can easily access the proxy URL.
